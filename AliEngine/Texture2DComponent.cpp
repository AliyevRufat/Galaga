...
void Texture2DComponent::Render()
{
	...
	if (m_HasAnimations)
	{
		dae::Renderer::GetInstance().RenderTexture(*m_spTexture2D, position.x, position.y, (float)m_SDLRect.w * m_Scale, (float)m_SDLRect.h * m_Scale, (float)m_SDLRect.x, (float)m_SDLRect.y, (float)m_SDLRect.w, (float)m_SDLRect.h);
	}
	else
	{
		int width = 0;
		...
		dae::Renderer::GetInstance().RenderTexture(*m_spTexture2D, position.x, position.y, (float)width * m_Scale, (float)height * m_Scale);
	}
};
